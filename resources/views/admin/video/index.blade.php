        ...
        <div class="container-fluid mt-4">
            ...
            <div class="page-header">
                ...
                <div class="header-action">
                    ...
                    <a href="{{ route('admin.video.create') }}" class="btn btn-danger btn-create d-flex align-items-center gap-2 text-decoration-none">
            ...
            <div class="card table-card">
                <div class="card-body">
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tr>
                            <td>{{ $video->id }}</td>
                            <td>{{ $video->user->name ?? 'Admin' }}</td>
                            <td>{{ $video->title }}</td>
                            <td>{{ $video->type === 'youtube' ? 'YouTube' : 'Upload File' }}</td>
                            <td>{{ $video->created_at->format('Y-m-d') }}</td>
                            <td>
                                <form action="{{ route('admin.video.updateStatus', $video->id) }}" method="POST" class="status-form">
                                    @csrf
                                    @method('PUT')
                            ...
                            <td class="text-end">
                                <a href="{{ route('admin.video.edit', $video->id) }}" class="btn btn-sm btn-outline-primary me-1" title="Edit">
