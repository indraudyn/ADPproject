...
class CeritaController extends Controller
{
    public function index(BackendApiService $apiService)
    {
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::warning("Gagal mengambil list cerita admin dari backend: " . $e->getMessage());
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::warning("Gagal mengambil list cerita lokal untuk admin: " . $e->getMessage());
        }
    ...
    public function show($id, BackendApiService $apiService)
    {
        if (strpos($id, 'local-') === 0) {
            ...
            try {
                ...
                \Illuminate\Support\Facades\Log::warning("Gagal mengambil detail cerita lokal untuk admin: " . $e->getMessage());
                abort(404);
            }
        }
        ...
        try {
            ...
            \Illuminate\Support\Facades\Log::warning("Gagal mengambil detail cerita admin dari backend: " . $e->getMessage());
        }
        abort(404);
    }
        ...
        if (strpos($id, 'local-') === 0) {
            ...
            try {
                ...
                try {
                    ...
                    \Illuminate\Support\Facades\Log::warning("Gagal mengambil kategori parwa untuk edit: " . $e->getMessage());
                    $parwas = \App\Models\Parwa::all()->map(fn($p) => (object)['book' => $p->name]);
                }
                return view('admin.cerita.edit', compact('cerita', 'parwas'));
            } catch (\Exception $e) {
                \Illuminate\Support\Facades\Log::warning("Gagal mengambil edit cerita lokal untuk admin: " . $e->getMessage());
                abort(404);
            }
        }
    ...
    public function update(Request $request, $id, BackendApiService $apiService)
    {
        ...
        if (strpos($id, 'local-') === 0) {
            ...
            try {
                ...
                return redirect()->route('admin.cerita.index')->with('success', 'Cerita lokal berhasil diperbarui');
            } catch (\Exception $e) {
        ...
        try {
            ...
            return redirect()->route('admin.cerita.index')->with('success', 'Cerita berhasil diperbarui di backend');
        } catch (\Exception $e) {
    ...
    public function updateStatus(Request $request, $id, BackendApiService $apiService)
    {
        ...
        if (strpos($id, 'local-') === 0) {
            ...
            try {
                ...
                \App\Services\ParwaCacheService::invalidateForBook($c->sumber);
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                if ($updateResp->successful()) {
                    \App\Services\ParwaCacheService::invalidateForBook($c['book'] ?? '');
                    return back()->with('success', 'Status cerita di backend berhasil diperbarui');
                }
            }
        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::warning("Gagal memperbarui status cerita di backend: " . $e->getMessage());
        }
    ...
    public function destroy($id, BackendApiService $apiService)
    {
        if (strpos($id, 'local-') === 0) {
            ...
            try {
                $c = \App\Models\Cerita::findOrFail($localId);
                $bookForCache = $c->sumber;
                $c->delete();
                \App\Services\ParwaCacheService::invalidateForBook($bookForCache);
                return redirect()->route('admin.cerita.index')->with('success', 'Cerita lokal berhasil dihapus');
            } catch (\Exception $e) {
        ...
        try {
            ...
            return redirect()->route('admin.cerita.index')->with('success', 'Cerita berhasil dihapus dari backend');
        } catch (\Exception $e) {
